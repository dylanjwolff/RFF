...
static Map* q_scores = NULL;
static uint64_t state_hash = 0;
...
static void sched_initialize(void)
{
    ...
    if (shm_id_env == nullptr)
    {
        ...
        q_scores = coopt(sizeof(Entry), (uint8_t *)sched_bitmap, SCHED_MAP_SIZE);
    }
    else
    {
    ...
    for (int i = rf_sched->pos_len; i < rf_sched->pos_len + rf_sched->neg_len; i++) {
        ...
        // negations
        // state_hash = hash_combine(state_hash, hash_combine(obs[i].rf.r, obs[i].rf.w + 1));
...
static void signal(void)
{
    ...
    for (auto item : *threads)
    {
        if (item->awake)
        {
            num_awake += 1;
            double q = 0;
            if (item->next_event.has_value()){
                q = -1 * get(q_scores, hash_combine(state_hash, (uintptr_t) item->next_event.value().event_id));
            } else {
                q = -1 * get(q_scores, hash_combine(state_hash, 0));
            }
            item->pos_priority = exp(q);
            denom += item->pos_priority;
        }
    }
    ...
    double cum_score = 0;
    THREAD* sampled_thread = nullptr;
    for (auto item : *threads)
    {
        if (item->awake)
        {
            if (sampled_thread == nullptr) {
                sampled_thread = item;
            }

            double score = item->pos_priority / denom;
            if (item->next_event.has_value()){
                fprintf(stderr, "-- score for %p: %f\n", item->next_event.value().event_id, score);
            }
...
bool update_state(Obligation* ob, Event e, bool will_exec_next) {
    ...
    if (!ob->is_negation) {
        if (ob->state != PosExctdW && !will_exec_next && ob->rf.r == (uintptr_t) e.event_id) {
            ...
            // negation
            // state_hash = hash_combine(state_hash, hash_combine(ob->rf.r, ob->rf.w));
            ob->count -= 1;
            new_state = PosNoInfo;
            ...
            if (e.is_write) {
                ...
                if (mayb_last_w_t != all_last_write_threads.end()) {
                    if (mayb_last_w_t->second != t.tid) {
                        ...
                        state_hash = hash_commute(state_hash, hash_combine((uintptr_t) e.event_id, last_w));
...
extern "C"
{
    ...
    uint64_t thread_switch(const void *instr_addr, const void *mem_addr, size_t size, bool is_write, const void *func, EventId *pending_obligation, bool is_negation)
    {
        ...
        uint64_t prev_state_hash = state_hash;

        record_rfs_if_needed(e);

        // update score
        uint64_t h = hash_combine(prev_state_hash, (uintptr_t) e.event_id); 
        double s = get(q_scores, h);

        // no maxQ... likely not important
        insert(q_scores, h, 0.7 * s  + 0.3 );
