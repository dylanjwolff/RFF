...
uint32_t placeholder_max_num_threads = 3;
uint32_t placeholder_max_num_events = 10000;
uint32_t placeholder_event_counter = 0;
uint32_t placeholder_thread_counter = 0;
...
inline size_t hash_commute(uintptr_t lhs,  uintptr_t rhs)
{
    return (lhs + rhs + 0x9e3779b9) ^ lhs ^ rhs;
}
...
static void sched_initialize(void)
{
    ...
    if (shm_id_env == nullptr)
    {
        ...
        max_num_threads = &placeholder_max_num_threads;
        max_num_events = &placeholder_max_num_events;
        thread_counter = &placeholder_thread_counter;
        event_counter = &placeholder_event_counter;
    }
    else
    {
        ...
        max_num_threads = (uint32_t*) sched_bitmap;
        max_num_events = (uint32_t*) (sched_bitmap + 4);
        thread_counter = (uint32_t*) (sched_bitmap + 8);
        event_counter = (uint32_t*) (sched_bitmap + 12);
    }
...
size_t pct_change_pt_ctr = 0;
std::vector<int> pct_priorities;
std::vector<int> pct_changed_priorities;
std::vector<uint32_t> pct_change_points;

static void pct_priorities_init() {

    for (int i=0; i<*max_num_threads; ++i) pct_priorities.push_back(i + max_depth);
    std::random_shuffle ( pct_priorities.begin(), pct_priorities.end() );
    for (int i=0; i<*max_num_threads; ++i) debug("%u -- pri\n", pct_priorities[i]);

    for (int i=0; i<max_depth-1; ++i) pct_change_points.push_back( rand() % (*max_num_events) );
    sort(pct_change_points.begin(), pct_change_points.end());
    for (int i=0; i<max_depth-1; ++i) debug("cp --- %u\n", pct_change_points[i] );

    for (int i=0; i<max_depth; ++i) pct_changed_priorities.push_back(i);
    std::random_shuffle ( pct_changed_priorities.begin(), pct_changed_priorities.end() );
    for (int i=0; i<max_depth; ++i) debug("%u -- changed pri\n", pct_changed_priorities[i]);
}
...
static void signal(void)
{
    ...
    for (auto item : *threads)
    {
        ...
        {
            num_awake += 1;
            if (pct_priorities[item->det_id % pct_priorities.size()] >= best_pri) {
                best_pri = pct_priorities[item->det_id % pct_priorities.size()];
                best_thread = item;
            }
    ...
    *event_counter += 1;
    if (pct_change_pt_ctr < pct_change_points.size() && (*event_counter) >= pct_change_points[pct_change_pt_ctr]) {
        debug("Change point %u, priority of thread %d from %u to %u\n",
             pct_change_pt_ctr,
             thread->tid,
             pct_priorities[thread->det_id % pct_priorities.size()],
             pct_changed_priorities[pct_change_pt_ctr % pct_changed_priorities.size()]);
        pct_priorities[thread->det_id % pct_priorities.size()] = pct_changed_priorities[pct_change_pt_ctr % pct_changed_priorities.size()];
        pct_change_pt_ctr += 1;
    }
